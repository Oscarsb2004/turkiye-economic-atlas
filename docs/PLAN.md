...
`data-2026-09-20c` (the election index the time slider reads), `data-2026-09-20d` (migration), `data-2026-09-20e` (airports), `data-2026-09-20f` (the railway), `data-2026-09-21` (İstanbul's transit). Each replays byte-identical on
its own recorded inputs; the current one is `data-2026-09-21`. A master whose sources include a LIVE database is re-recorded when that database moves: the OSM railway changed by one station name between T9 and this recording, which the record's own `committed-vs-recorded` reported.
